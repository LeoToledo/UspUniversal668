...
#Escolhe qual ação tomar(aleatória ou não)
    ...
    def play(self, current_state, eps):
            ...
            for i in range(self.iteration_num):
                ...
                if done:
                    pf(print("Episodio: ", eps," || ", i, " Steps", " || Reward: ", reward_sum, " || EPSILON: ", self.epsilon), flush=True)
                    self.train_network.save('./TrainedNet', eps, 'h5')
                    break
...
def plot(reward_store):
    #Fazendo uma média das rewards
        ...
        plt.plot(moving_aves, color='r',linewidth=2)
